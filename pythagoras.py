...
def frame(r, theta, a, b, side_movement, circle1_start, c1e):
  ...
  adj_pointing, _ = tf.linalg.normalize(side_movement * adj_pointing2 + (1-side_movement) * adj_pointing1, axis=-1)

  opp_pointing1 = far - corner
  opp_pointing2 = far - origin

  opp_pointing, _ = tf.linalg.normalize(side_movement * opp_pointing2 + (1-side_movement) * opp_pointing1, axis=-1)
...
class Drawing(tf.Module):
  def __init__(self):
    super().__init__()
    self.c1e = tf.Variable(1., trainable=False)
