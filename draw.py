...
def boltzmann(a, x):
    return tf.reduce_sum(x * tf.exp(a * x), axis=0) / tf.reduce_sum(
        tf.exp(a * x), axis=0
    )
...
red = pure_color(tf.constant([1.0, 0, 0, 1]))
green = pure_color(tf.constant([0.0, 1, 0, 1]))
blue = pure_color(tf.constant([0.0, 0, 1, 1]))
white = pure_color(tf.constant([1.0, 1, 1, 1]))
black = pure_color(tf.constant([0.0, 0, 0, 1]))
...
def composite(bf, af):
    def render(x):
        ...
        return tf.concat(
            [rgb_a + rgb_b * (1 - alpha_a), alpha_a + alpha_b * (1 - alpha_a)], axis=-1
        )
